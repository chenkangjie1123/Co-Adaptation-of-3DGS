...
dataset_name = "LLFF"
data_base_path='./data/nerf_llff_data_colmap'
out_base_path=f'./output/{dataset_name}'
n_views = 3 # 3
resolution = 2
dropout_factor = 0.3 # 0.2 or 0.3 sometimes 0.3 better 
# Due to some small coding difference 
# in our initial version 0.2 performs better
# so we report p=0.2 in our paper
sigma_noise = 0.0 # 0.8 0.4 0.2 0.05 
# The opacity noise strategy is not as robust as the dropout strategy. 
# For different methods or data sets, the effects of different degrees of noise injection vary. 
...
def train_block(gpu_id, scene):
    ...
    # 2. training 3DGS
    cmd = (f'CUDA_VISIBLE_DEVICES={gpu_id} python train.py -s {data_base_path}/{scene} -m {model_path} '
           f'--n_views {n_views} --dataset_name {dataset_name} --resolution {resolution} '
           f'--dropout_factor {dropout_factor} --sigma_noise {sigma_noise} --eval')
    print(cmd)
    os.system(cmd + f" >> {log_file} 2>&1")
        ...
        if os.path.isdir(scene_path):
            ...
            if os.path.exists(results_path):
                try:
                    ...
                    psnr_avg += results["ours_30000"]["PSNR"]
                    ssim_avg += results["ours_30000"]["SSIM"]
                    lpips_avg += results["ours_30000"]["LPIPS"]
